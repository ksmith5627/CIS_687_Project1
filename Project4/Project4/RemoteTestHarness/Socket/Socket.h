...
namespace Sockets
{
	...
	*/
	...
	bool SocketListener::Start(CallableObject& co)
	{
		...
		typename std::thread listenThread( //ksnote -  added type name to fix the compile error 
			[&]()
		{
			while (!_acceptFailed)
			{
				...
				typename std::thread clientThread(co, std::move(clientSocket)); //ksnote -  added typename to resoolve a comile error
